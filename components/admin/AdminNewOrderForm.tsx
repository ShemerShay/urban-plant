...
export function AdminNewOrderForm() {
  ...
  const checkoutErrors = translateCheckoutFieldErrors(

    locale,

    getVisibleCheckoutFieldErrors(checkoutFieldErrors, touched, showAllErrors),

  );

  const visibleAdminErrors = getVisibleAdminNewOrderFieldErrors(
  ...
  const adminErrors = {
    ...
    plantId: visibleAdminErrors.plantId

      ? t(locale, visibleAdminErrors.plantId)

      : undefined,

    price: visibleAdminErrors.price ? t(locale, visibleAdminErrors.price) : undefined,
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setSubmitError(displayApiError(locale, data.error, "admin.orders.createFailed"));
        ...
        >
          ...
          {mockPlants.map((plant) => (
            ...
            <option key={plant.id} value={plant.id} className="text-slate-900">
              ...
              {plant.name} ({formatPrice(plant.price, plant.currency, locale)})
      ...
      <div className="relative">
        ...
        {showValidationOverlay ? (
          ...
          <button
            ...
            aria-label={t(locale, "admin.orders.showRequired")}
        ...
        >
          ...
          {isSubmitting ? t(locale, "admin.shared.saving") : t(locale, "admin.orders.save")}
